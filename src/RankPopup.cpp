...
bool RankPopup::canShow() {
    if (!CCScene::get() || GJBaseGameLayer::get())
        return false;

    return true;
}

void RankPopup::queueRankChange(int oldRank, int newRank) {
...
void RankPopup::showRankChange(int oldRank, int newRank) {
    ...
    int distance = std::abs(newRank - oldRank);
    ...
    if (difference < 0) {
        m_change->setString(fmt::format("+{}", -difference).c_str());
        m_change->setColor({0, 255, 120});
    }
